...
def capturarDatos():
    cuotas = cuotas_entry.get()
    tasa_interes = tasa_entry.get()
    ...
    print(Renta.calcularValor(float(tasa_interes), int(cuotas),date(piy,pim,pid), date(pfy,pfm,pfd)))
...
enviar_button = ctk.CTkButton(master=frame, text="Calcular", font=("Roboto", 16),command=capturarDatos)
enviar_button.grid(row=5, column=1, columnspan=3, pady=10, padx=8)
